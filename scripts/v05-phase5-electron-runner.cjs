...
app.setPath('userData', path.join(os.tmpdir(), 'wb-v05p5-userdata-' + Math.random().toString(36).slice(2)));

const rootDir = path.resolve(__dirname, '..');
const distIndexPath = path.join(rootDir, 'dist', 'index.html');
const suitePath = path.join(__dirname, 'v05-phase5-suite.js');
...
/** A path that is guaranteed not to exist: create a temp dir, then remove it. */
function makeDeadPath(prefix) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), prefix));
...
app.whenReady().then(async () => {
  const fixtureDir = makeFixture('wb-v05p5-fixture-');
  const fixtureDir2 = makeFixture('wb-v05p5-fixture2-');
  const deadDir = makeDeadPath('wb-v05p5-dead-');
  ...
  try {
    ...
    const testResult = await win.webContents.executeJavaScript(`
      ...
      window.__testDeadDir = ${JSON.stringify(deadDir)};
      ${suiteCode}
      window.__runV05Phase5Suite();
    `);
    console.log('[electron] v0.3 Phase 5 Results: ' + JSON.stringify(testResult));
    assert(testResult && testResult.success === true, 'All v0.3 Phase 5 Electron assertions passed');
  } catch (err) {
