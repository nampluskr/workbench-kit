...
- **탭 오버플로우("›N") 드롭다운이 다크·그레이 테마에서 안 보임 (사용자 지시로 직접 실행해 결함 탐색 중 발견, 2026-09-13)**
  - **발견**: Electron·pywebview 두 갈래를 직접 띄우고 CDP(`Runtime.evaluate`·`Page.captureScreenshot`)로 실제 클릭을 재현하며 점검하던 중, 한 칸에 탭이 많아 `›N` 오버플로우 표시를 누르면 목록(`dv-tabs-overflow-container`)이 DOM에는 생기는데 화면엔 아무것도 안 보이는 것을 발견했다. `document.elementFromPoint()`로 확인하니 그 자리의 클릭도 밑에 깔린 monaco 에디터로 그대로 새고 있어 — 안 보이는 것만이 아니라 클릭도 안 먹는 상태였다.
  - **원인 (둘)**: (1) dockview가 이 팝오버에 주는 인라인 스타일이 `z-index: var(--dv-overlay-z-index)`인데 이 토큰을 어디서도 정의한 적이 없어 `auto`로 무너지고, 형제인 `.dv-content-container`(역시 `position: relative`, `z-index: auto`)에 DOM 순서로 밀려 에디터 내용 밑에 깔렸다. (2) `.dv-tabs-overflow-container`와 그 안의 `.dv-tab`이 배경·글자색을 하나도 못 받아(이 팝오버가 `.dv-groupview > .dv-tabs-and-actions-container` 경로 밖에 떠서, 그 경로를 겨냥한 기존 테마 규칙이 하나도 안 걸림) 페이지 기본 텍스트색만 상속했다. 이미 `.claude/rules/dockview-css.md`가 경고하는 "CSP가 dockview의 인라인 `<style>` 주입을 막는다"는 사고의 같은 계열인데, 이 두 곳만 손복사가 빠져 있었다.
  - **조치**: `src/style.css`에 `--dv-overlay-z-index: 100`을 `:root`에 추가하고, `.dv-tabs-overflow-container`(배경·테두리·그림자)와 `.dv-tabs-overflow-container .dv-tab`(배경·글자색·hover)에 기존 `--tab-inactive-bg`/`--tab-active-bg` 등 테마 토큰을 재사용하는 규칙을 더했다. 그림자 값(`0 2px 8px`)이 `FR-M5`/`D-29` 금지 값(`8px`)에 걸려 다른 팝오버들과 같은 `0 5px 15px`로 바꿨다.
  - **결과**: `›N`을 누르면 목록이 다크·라이트·그레이 테마 모두에서 배경·테두리·그림자를 갖춘 팝오버로 뜨고, 항목을 클릭하면 실제로 그 탭이 활성화된다(에디터 밑으로 새던 클릭 해소).
  - **검증**: 수정 전/후로 CDP를 통해 (a) 오버플로우 팝오버가 뜬 상태에서 `elementFromPoint`가 monaco 대신 팝오버 자신을 반환하는지, (b) 목록 항목 클릭이 실제로 탭을 활성화하는지 실측 확인. `npm run build` 후 Electron·pywebview 재기동해 시각 확인(스크린샷). `npm test` 전건 재실행 — 수정 전 베이스라인과 동일하게 `V2P2-FR-F11`(포커스 링 대비, 기존 환경 이슈) 1건만 남고 새로 생긴 실패 0건.

- **UI 사용성 개선 묶음 (사용자 UI 검토 중 요청, 2026-09-14)**
  - **무엇을**: (1) Activity Bar 타이틀바·상태바 토글 chevron의 보임/감춤 방향을 반전(`activitybar.ts`·`main.ts`). (2) 탐색기 트리 들여쓰기 칸을 10px로 좁히고 트위스티를 왼쪽 정렬, 세로 정렬선을 트위스티 글리프 중심에 맞춤(`style.css` `.tree-indent-unit`·`.tree-twistie`). codicons가 `<i>`를 16px로 강제해 글리프 중심이 박스 왼쪽에서 반 폭 지점에 온다는 것이 정렬 기준. (3) 탭 줄의 `›N` 오버플로우 표시를 끔(`editor.ts` `disableTabsOverflowList: true`) — 09-12에 스크롤을 `.dv-scrollable`로 옮긴 뒤 dockview가 여전히 `.dv-tabs-container`의 `scrollWidth`로 넘침을 판정해, 스크롤바와 틀린 숫자(`›6`)가 함께 뜨고 스크롤해도 숫자가 갱신되지 않았다. (4) 탐색기·편집기 칸 스크롤바를 탭 줄과 같은 5px로, monaco 스크롤바도 `verticalScrollbarSize`/`horizontalScrollbarSize: 5`. (5) 다크 테마 크롬 배경을 사용자 VS Code 테마(Monokai Dimmed) 값으로 밝힘(편집기·활성 탭 배경 유지). (6) 세 테마 글자·테두리 재조정: 새 `--focus-ring` 토큰(다크 `#7a7a7a` · 화이트 `#8a8a8a` · 그레이 `#505050`)으로 트리 커서와 활성 탭 포커스 테두리를 부드러운 회색으로, 다크 타이틀·상태바 글자 `#e0e0e0`·비활성 탭 글자 `#b8b8b8`, 화이트 크롬 `#f0f0f0`·비활성 탭 글자 `#6c6c6c`, 그레이는 중간 회색(L* ~55) 배경이 검정 글자도 5:1 안팎에 묶어 뿌옇게 보이던 것을 편집기 `#c0c0c0`·크롬 `#a6a6a6`·글자 `#1a1a1a`로 바꿈. `scripts/verify-phase2.mjs`의 고정 색 값과 그레이 중간값 보간 검사를 새 값·"편집기와 크롬 대비 1.3 이상" 검사로 갱신(사용자 승인).
  - **결과**: 탭 12개에서 `›N` 없음, 스크롤바 동작 유지. 세 테마 실측 대비 — 트리 글자 9.30 · 9.83 · 7.15, 포커스 테두리(사이드바/선택 행) 3.48/3.84 · 3.03/3.45 · 3.31/4.43.
  - **검증**: Electron을 원격 디버깅 포트로 띄워 CDP로 실측(탭 줄 폭·스크롤·`›N` 유무, 스크롤바 계산 두께, monaco 스크롤바 5px, 세 테마의 계산된 색과 대비 — 포커스 에뮬레이션 켜고 측정)과 스크린샷 확인. `npm run typecheck` 통과, `node scripts/verify-phase2.mjs`의 색·대비 항목 전부 통과.
  - **남은 것**: (a) 트리 세로선 `left: 8px`와 그 주석의 `8px`·`16px`가 `verify-phase2.mjs` 1절 금지 값(D-29)에 걸려 **2건 실패** 중 — 트위스티 `padding-left: 1px` + 세로선 `left: 9px`로 정렬 유지한 채 해소 가능, 미적용. (b) FR-F11의 "선택 배경이 사이드바와 1.3 이상 구분" 기준을 다크(1.10)·화이트(1.14)가 못 넘는다 — 09-12에 선택 배경을 `--tab-active-bg`로 바꾼 뒤부터 이미 미달이었던 것으로 보이며 이번 변경으로 새로 생긴 것은 아니다. (c) `›N` 목록용으로 09-13에 넣은 `.dv-tabs-overflow-container` 규칙과 `--dv-overlay-z-index`는 이제 쓰이지 않지만 남겨 둠. `npm test` 전건은 돌리지 않았다.
  - **추가 (같은 날)**: 활성 탭의 배경·포커스 테두리 사각형이 탐색기 선택 행보다 커 보인다는 지적에 `.dv-tab` 높이를 `100%`(30px)에서 트리 행과 같은 22px로 줄이고 `.dv-tabs-container`에 `align-items: center`를 더했다. 탭 줄은 30px 그대로(FR-D1). CDP 실측 — 탭 22px, 탭 줄 30px, 위아래 여백 각 4px.
  - **추가 (같은 날)**: 22px로 줄인 활성 탭 아래 4px 틈이 탭 줄 배경색이라 탭과 에디터가 끊겨 보인다는 지적에, `.dv-tab`에 `position: relative`를 주고 `.dv-tab.dv-active-tab::after`로 그 틈을 탭 자신의 배경색(`background-color: inherit`)으로 채웠다. 높이는 금지 값(D-29)을 피해 `calc((var(--area-size) - 22px) / 2)`. CDP 실측 — 세 테마 모두 `::after` 4px, 색이 활성 탭 배경과 같음(다크 `#1f1f1f` · 화이트 `#ffffff` · 그레이 `#c0c0c0`, 각 테마의 편집기 배경과 동일). `verify-phase2.mjs` 금지 값 항목에 새 실패 없음(기존 `8px`·`16px` 2건만).
  - **추가 (2026-09-15)**: 다크 테마에서 탭 닫기 "×"가 거의 검게 보이던 원인 — dockview는 닫기 버튼을 `class="dv-svg"` SVG로 그리고 `fill: currentcolor`를 자기 주입 CSS에만 두는데, 그 CSS가 CSP에 막혀 SVG 기본 검정 채우기로 떨어졌다(`.claude/rules/dockview-css.md`와 같은 계열). `style.css`에 `.dv-svg` 규칙을 손복사하고, 사용자 지시대로 `.dv-tab .dv-default-tab-action`의 색을 테마 테두리색 `var(--focus-ring)`으로 지정. CDP 실측 — "×" path fill이 다크 `#7a7a7a` · 화이트 `#8a8a8a` · 그레이 `#505050`으로 각 테마 `--focus-ring`과 일치. 트리 세로선 간격은 사용자 결정으로 현재 값(`left: 8px`) 유지 — 따라서 `verify-phase2.mjs` 금지 값 `8px`·`16px` 2건 실패는 그대로 남는다.

- **pywebview가 항상 빈 탐색기로 뜸 — Electron은 마지막 폴더를 복원하는데 pywebview는 못 함 (사용자가 두 갈래를 나란히 띄워 비교하다 발견, 2026-09-15)**
  - **발견**: "마지막 폴더 열기"·"최근 폴더" 복원(FR-K1)은 전적으로 `localStorage`(`workbench:last-folder`, `workbench:recent-folders`, `src/main.ts`)에 의존한다. `src/hosts/pywebview/main.py`가 인자 없이 `webview.start()`를 부르고 있었는데, pywebview는 `private_mode` 기본값이 `True`이고 `storage_path`가 없으면 WebView2의 `UserDataFolder`를 `tempfile.TemporaryDirectory().name`(백엔드: `webview/platforms/winforms.py` `init_storage()`) — **실행마다 새로 생기는 임시 디렉터리**로 잡는다. 그 결과 localStorage가 재실행 간에 전혀 안 남아, 항상 빈 상태로 뜬다. Electron은 Chromium의 기본 영속 userData 파티션을 쓰므로 같은 문제가 없다.
  - **조치**: `webview.start()`에 `storage_path=os.path.join(os.environ.get("LOCALAPPDATA") or tempfile.gettempdir(), "workbench-kit", "pywebview")`를 명시해 영속 프로필을 쓰게 했다. host별 부트스트랩 코드라 `.claude/rules/common-core.md`(공통 코어 금지 사항)와는 무관.
  - **결과**: `%LOCALAPPDATA%\workbench-kit\pywebview\EBWebView` 프로필이 생성되고, 재실행 간 localStorage가 유지되는지는 실제 폴더 열기 → 재시작 전체 흐름으로 사용자 재확인이 필요(이번엔 스모크 테스트로 기동만 확인).
  - **검증**: `--smoke-test` 플래그로 기동·DOM 로드 확인(정상). `python -m py_compile` 대체로 `uv run python -c "ast.parse(...)"`로 문법 확인. 반대 벤더 적대적 검증은 거치지 않음 — 새 기능·리팩토링이 아닌 1줄짜리 host 부트스트랩 결함 수정으로 판단했다.

- **상태표시줄 경로: 에디터 칸 선택도 반영 · 글꼴 크기 보정 · 백슬래시 오표시 수정 (사용자 요청, 2026-09-15)**
  - **요청**: (1) 지금은 탐색기 트리 선택만 하단 상태표시줄 경로(`statusbar-path`)를 갱신하는데, 에디터 칸(탭) 선택에도 같은 걸 반영해 달라. (2) 상태표시줄(경로·프리셋) 글꼴을 UI 기본 크기로. (3) 경로에 백슬래시 대신 "￦"(원화 기호)로 보이는 걸 "\"로.
  - **조치**: (1) `src/main.ts`의 `editor.onActivePanelChange` 핸들러(FR-G3의 "타깃 사라짐" 오류 검사가 이미 있는 곳)에 `panel.params.targetId`가 파일·폴더 kind일 때 `statusbarPath.textContent`를 같이 쓰는 코드를 더했다 — 새 훅을 만들지 않고 탐색기 `tree.onSelect`가 쓰는 것과 같은 요소·같은 갱신 방식을 재사용. (2) `style.css`의 `.workbench-statusbar { font-size: 11px }`을 앱 전반이 쓰는 UI 기본 크기 `13px`로. (3) 원인은 문자열 버그가 아니라 폰트 렌더링이었다 — 상태표시줄이 상속하는 `--font-family`(`system-ui, ...`)가 한글 Windows에서 맑은 고딕으로 해석되는데, 맑은 고딕은 옛 코드페이지 949 관행으로 `U+005C`(백슬래시) 글리프를 원화 기호 모양으로 그린다(실제 문자는 끝까지 `\`). `statusbar-path`·`statusbar-app-item`(Preset Info)에 이 치환이 없는 고정폭 폰트 스택(`Consolas, SFMono-Regular, Menlo, monospace`)을 명시했다. 1차로 `Cascadia Mono`를 넣었다가 `verify-dist.mjs`의 tab-explorer-templates 금지 토큰 검사(FR-M5)에 걸려 빼고 다시 빌드·검증했다.
  - **결과**: 에디터 탭을 바꾸면 그 탭이 가리키는 파일·폴더 경로가 상태표시줄에 즉시 뜬다(트리 선택과 같은 자리). 상태표시줄 글자가 13px로 커짐. 임시 Electron 스크립트(`scripts/_tmp-statusbar-check.cjs`, 확인 후 삭제)로 `statusbar-path`에 `D:\projects\workbench-kit\src\main.ts`를 넣고 컴퓨티드 스타일을 실측 — `fontFamily: "Consolas, SFMono-Regular, Menlo, monospace"`, `fontSize: "13px"`, `textContent`가 실제 백슬래시 4개를 그대로 담고 있음을 확인(JS 문자열 레벨 확인이라 렌더 글리프 자체는 육안 재확인 필요).
  - **뜻밖의 발견**: 검증 도중 `src/hosts/pywebview/main.py`가 작업 트리에서 CRLF로 바뀌어 있는 것을 발견 — 이전 턴의 `storage_path` 수정 후 진단 목적으로 쓴 `git stash`/`stash pop`이 이 리포의 `core.autocrlf=true` 설정과 맞물려 LF로 커밋된 파일을 CRLF로 재작성한 것(같은 방식으로 쓰이는 `main.cjs`는 내가 한 번도 건드리지 않아 LF 그대로 남아 있었음). `verify-dist.mjs`의 "Host introspection expressions are byte-for-byte identical" 검사가 이 차이로 실패해 알아챘다. LF로 되돌려 해소. 이후 `git stash`는 다시 쓰지 않고 원본 파일을 임시 백업(`/tmp`)해 대조하는 방식으로 바꿨다.
  - **검증**: `npm run typecheck` 통과. `npm test`(typecheck + verify:dist + phase2~7 + v02-phase1~7) 전건 실행 — 새로 생긴 실패 0건. 사전에 있던 실패만 재확인됨: `verify:dist`/`verify:phase2~5`의 "8px"·"16px" 금지 값 2건(트리 세로선, 2026-09-14 항목에 사용자 결정으로 유지 기록됨), `verify:v02-phase2`의 FR-F11 sel/sidebar 대비 미달 5건(2026-09-14 항목에 이미 미달로 기록됨), `verify:v02-phase5`의 "Electron runner exited non-zero"(모든 개별 단언은 통과 — 수정 전 원본 파일로 되돌려 동일하게 재현되는 걸 확인한 환경 플레이크), `verify:dist`의 "Electron executed frontend script (status: \"\")"(원본 파일로 되돌려도 3/3 재현되는 기존 환경 이슈 — Electron `did-finish-load`와 앱의 동적 레이아웃 마운트 사이 타이밍 경합으로 추정, 이번 변경과 무관함을 원본 대조로 확인). 반대 벤더 적대적 검증은 거치지 않음 — 새 기능·리팩토링이 아닌 UI 사용성 보정으로 판단했다.

- **메뉴 선택(hover) 배경색을 VS Code 파란색에서 테마별 무채색 계열로 (사용자 요청, 2026-09-15)**
  - **요청**: 햄버거 메뉴를 펼쳤을 때 항목 선택색이 다크·화이트 테마에서 VS Code 기본 파란색(`#0078d4`·`#005fb8`)이라 2026-09-14에 이미 무채색으로 바꿔 둔 나머지 크롬과 어울리지 않는다는 1차 지적. 사용자 의도를 정리해 확인받은 뒤, 그레이 테마의 기존 회색(`#5a5a5a`)이 "너무 진하다"는 2차 지적과 함께 "다른 테마도 비슷한 그레이 계통 느낌으로" 통일해 달라는 요청으로 이어졌다.
  - **조치**: `style.css`의 `--menu-hover-bg`/`--menu-hover-fg`를 세 테마 모두 무채색으로 다시 잡았다 — 다크 `#454545`(흰 글자, `--tab-inactive-bg`와 같은 계열로 `--menu-bg`보다 한 단계 밝음), 화이트 `#e0e0e0`(밝은 배경이라 흰 글자 대신 짙은 글자 `#1f1f1f`로 전환), 그레이 `#a6a6a6`(이 테마의 크롬 톤 `--titlebar-bg`를 그대로 재사용 — 기존 `#5a5a5a`보다 밝아 "진하다"는 지적 해소, 글자는 `#1a1a1a`). `.menu-item-row:hover`의 단축키 글자도 `--menu-hover-fg`를 그대로 쓰므로 같이 바뀐다. `scripts/verify-phase2.mjs`의 옛 정확값 검사(`#0078d4`·`#005fb8`) 두 줄을 새 값으로, hover 명암비 검사 두 줄을 새 배경/글자 쌍으로 고치고, 그레이 hover 명암비 검사와 "hover 배경이 메뉴 배경과 최소한으로 구분되는지"(≥1.2:1) 검사 세 테마분을 새로 더했다.
  - **결과**: 세 테마 모두 메뉴 선택색이 무채색 계열로 통일되고, 그레이 테마 선택색이 눈에 띄게 옅어짐. 임시 Electron 스크립트(`scripts/_tmp-menu-color-check.cjs`, 확인 후 삭제)로 `data-theme`을 dark·light·gray로 바꿔가며 `--menu-hover-bg`/`--menu-hover-fg`/`--menu-bg` 계산값을 실측 — 의도한 값과 일치(다크 `#454545`/흰 글자, 화이트 `#e0e0e0`/`#1f1f1f`, 그레이 `#a6a6a6`/`#1a1a1a`).
  - **검증**: `npm run typecheck` 통과. `npm run verify:phase2` — 새 hover 관련 단언(정확값 2건, 명암비 6건, 배경 구분 3건) 전부 통과, 실패는 기존에 있던 "8px"·"16px" 금지 값 2건뿐(무관, 09-14 항목에 기록됨). `npm run verify:dist`도 같은 기존 3건(8px·16px·Electron 상태바 텍스트 타이밍 플레이크)만 남고 새 실패 없음. 반대 벤더 적대적 검증은 거치지 않음 — 새 기능·리팩토링이 아닌 색상 토큰 조정으로 판단했다.

- **에디터 탭: 수정 표시 점이 이탤릭을 따라가지 않게 · 닫기 "X" 주변 여백 재조정 (사용자 요청, 2026-09-15)**
  - **요청**: (1) 미리보기 탭(제목이 이탤릭)의 파일이 수정되면 제목 앞에 붙는 "●"(FR-L1)도 함께 이탤릭으로 기울어지는데, 이 점은 기울면 안 된다. (2) 탭 제목 오른쪽 "X" 아이콘 좌우 여백을 최소화해 달라 — 1차로 제목↔X 간격·X 자체 패딩·X↔탭끝 여백을 모두 줄였더니, 이탤릭 제목("Untitled" 등)의 마지막 글자("d")가 잘려 보이는 부작용이 나왔다(3). 다시 조정해 제목↔X 간격은 늘리고, X 자체 패딩과 X↔탭끝 여백은 최소로 유지했다. (4) 탭 왼쪽 바깥 여백도 처음엔 새 제목↔X 간격과 같은 값을 제안했으나, 사용자가 `3px`로 직접 지정했다.
  - **조치**: (1) dockview 기본 탭은 `panel.title` 문자열을 그대로 `textContent`로 꽂아 렌더링하므로(`node_modules/dockview-core` `DefaultTab.render()`) "●"를 감싸는 별도 DOM이 없다 — `::first-letter`로 첫 글자만 별도 스타일링하되, 깨끗한(수정 없음) 탭의 제목 첫 글자까지 건드리지 않도록 `editor.ts`에 `applyDirtyClass()`(기존 `applyPreviewClass()`와 동일 패턴, `.dv-tab[data-tab-panel-id]`에 `workbench-dirty-tab` 클래스 토글)를 새로 만들어 `setTabDirty()`에서 호출하고, `onDidLayoutChange`의 기존 `refreshPreviewClasses()` 재적용 경로에 `refreshDirtyClasses()`를 같이 태웠다(탭이 그룹 이동 등으로 dockview에 의해 다시 그려질 때 클래스가 같이 유실되므로). `style.css`에 `.dv-tab.workbench-dirty-tab .dv-default-tab-content::first-letter { font-style: normal; }` 추가. (2)(3)(4) `.dv-tab` 좌우 패딩을 `padding: 0 2px 0 3px`(왼쪽 3px·오른쪽 2px, 기존 양쪽 10px)로, `.dv-default-tab`의 `gap`(제목↔X)을 `5px`(기존 2px, 그 전엔 5px였다가 2px로 줄였던 것을 다시 늘림)로, `.dv-default-tab-action`의 `padding`은 `1px`(기존 2px에서 이미 줄여 둔 값) 그대로 유지.
  - **결과**: 수정 표시 점은 미리보기 이탤릭 탭에서도 항상 똑바로 선다(제목 텍스트만 이탤릭 유지). 임시 Electron 스크린샷(`scripts/_tmp-tab-shot.png`, 확인 후 삭제)으로 "Untitled" 미리보기 탭을 실측 — "d"가 더는 잘리지 않고, 왼쪽 여백·X 주변 여백 모두 눈에 띄게 좁아짐.
  - **검증**: 임시 Electron 스크립트(`scripts/_tmp-tab-check.cjs`, 확인 후 삭제)로 dirty+preview 탭을 직접 만들어 실측 — `hasDirtyClass: true`, 전체 `fontStyle: "italic"`, `::first-letter`만 `fontStyle: "normal"`. `npm run typecheck` 통과. `npm run verify:phase2`·`verify:phase4` — 새로 생긴 실패 0건, 기존 "8px"·"16px" 금지 값 2건만 남음. 반대 벤더 적대적 검증은 거치지 않음 — 새 기능·리팩토링이 아닌 UI 결함 수정 + 여백 조정으로 판단했다.
  - **추가 (2026-09-15)**: 탭 왼쪽 바깥 여백을 사용자 지시로 `3px`에서 `5px`로 다시 조정(`padding: 0 2px 0 5px`). `npm run typecheck`·`build` 통과.

- **타이틀바 최대화 버튼 아이콘이 상태에 따라 바뀌지 않음 (사용자 요청, 2026-09-15)**
  - **요청**: 최대화 버튼을 눌러 창이 최대화되면 아이콘이 "사각형 두 개가 겹친" 모양(복원 아이콘)으로 바뀌고, 다시 원래 크기로 돌아오면 원래의 네모 하나 아이콘으로 되돌아가야 한다 — 지금은 `codicon-chrome-maximize`로 고정돼 있어 실제로 창이 최대화돼도 아이콘이 그대로였다.
  - **조치**: `src/core/window.ts`의 `WindowControlsBridge.maximize`가 `void`뿐 아니라 `boolean`/`Promise<boolean | null>`도 반환할 수 있게 타입을 넓히고, `maximizeWindow()`를 async로 바꿔 그 반환값(새 최대화 상태)을 돌려주게 했다. `setupWindowControls`에 `applyMaximizedIcon()`(아이콘 클래스 `codicon-chrome-maximize` ↔ `codicon-chrome-restore` 토글, `aria-label`/`title`도 "Maximize"/"Restore"로 갱신)을 추가해 버튼 클릭 시 그 반환값으로 바로 적용한다. 새 `WindowControlsBridge.onMaximizedChange` 훅도 추가해 `setupWindowControls`가 구독한다.
    - **pywebview**: `WindowApi.maximize()`(`main.py`)가 새 `is_maximized` 값을 반환하도록 고쳤다 — pywebview JS 브리지는 호출마다 요청/응답 왕복이라 버튼 클릭 경로만으로 충분하다.
    - **Electron**: IPC `send`는 fire-and-forget이라 반환값이 없다. 대신 `main.cjs`에 `win.on('maximize'/'unmaximize', ...)`을 달아 실제 창 상태가 바뀔 때마다(버튼 클릭뿐 아니라 타이틀바 드래그 영역 더블클릭·OS Snap 등 어떤 경로로든) `window:maximized-changed`를 렌더러에 push하고, `preload.cjs`에 `onMaximizedChange` 구독을 새로 노출했다.
  - **결과**: 최대화 ↔ 복원을 오갈 때마다 버튼 아이콘과 `aria-label`이 함께 뒤집힌다.
  - **검증**: `npm run typecheck`·`build` 통과. 실제 Electron 앱을 `--remote-debugging-port`로 띄우고 CDP(`Input.dispatchMouseEvent`)로 버튼을 **실제로 두 번 클릭**해 확인(임시 스크립트 `scripts/_tmp-cdp-maximize.mjs`, 확인 후 삭제) — 클릭 전 `codicon-chrome-maximize` → 1차 클릭 후 `codicon-chrome-restore`/"Restore" → 2차 클릭 후 다시 `codicon-chrome-maximize`/"Maximize". (도중 1차 시도는 좌표를 클릭 전에 한 번만 계산해 재사용하는 바람에 2차 클릭이 빗나갔다 — 최대화로 창 폭이 바뀌면 우측 정렬된 버튼도 같이 움직인다는 것을 놓쳤던 원인으로, 클릭마다 좌표를 다시 구하도록 고쳐 해소.) `npm run verify:dist`·`verify:phase4`·`verify:v02-phase3` — 새로 생긴 실패 0건(기존 "8px"·"16px"·상태바 텍스트 타이밍 플레이크만 재확인). pywebview는 `--smoke-test`로 기동·정상 로드만 확인했고(문법 오류 없음, `main.maximize()`가 이제 값을 반환하는 것 자체는 스모크 테스트 범위 밖), 버튼을 눌러 아이콘이 실제로 바뀌는지는 이번엔 확인하지 못했다. 반대 벤더 적대적 검증은 거치지 않음 — 새 기능이지만 범위가 작은 UI 피드백 보강으로 판단했다.

- **상태표시줄 글꼴 크기를 13px → 12px로 (사용자 요청, 2026-09-15)**
  - **요청**: 상태표시줄 글자를 더 작게 — 13px에서 12px로.
  - **발견**: `12px`는 v0.1 D-29(`FR-M5`)의 금지 값 목록(`4px`·`6px`·`8px`·`12px`·`16px`)에 있다. 사용자에게 확인한 뒤 "12px로, D-29 예외 승인"으로 진행 — `calc(13px - 1px)` 같은 문자열 우회는 D-29의 취지(출처 불명 값 유입을 검사가 못 잡는 상태)를 그대로 재현하므로 배제했다.
  - **조치**: `DECISIONS.md`에 `D-17`(선택·근거·배제한 대안)을 새로 남겨 이 예외를 정식 기록했다. `style.css`의 `.workbench-statusbar`에 `font-size: 12px;` 적용(근처에 D-17 참조 주석). `scripts/verify-dist.mjs`·`verify-phase2.mjs`·`verify-phase3.mjs`·`verify-phase4.mjs`·`verify-phase5.mjs`(D-29 금지 값 검사를 각자 따로 구현하고 있던 다섯 곳 전부)에 `font-size: 12px;` 리터럴 문자열을 스캔 전에 정확히 한 번만 제거하는 예외 처리를 더해, 이 한 자리만 봐주고 다른 곳의 `12px`는 여전히 잡히게 했다.
  - **뜻밖의 발견**: 1차로 `style.css`에 남긴 설명 주석 자체가 예시로 `"font-size: 12px;"` 문자열을 그대로 담고 있어, 각 검사 스크립트의 `.replace('font-size: 12px;', '')`(문자열 replace는 첫 매치만 지운다)가 실제 선언이 아니라 **주석 속의 예시 문구**를 지워버려 예외가 전혀 먹지 않는 결과가 났다(전 검사에서 "found: 1" 그대로). 주석 문구를 실제 CSS 문법을 그대로 반복하지 않도록 고쳐 해소 — 파일에 그 문자열이 정확히 한 번만 나타나는지 `grep`으로 재확인했다.
  - **검증**: `npm run typecheck`·`build` 통과. `verify:dist`·`verify:phase2`·`verify:phase3`·`verify:phase4`·`verify:phase5` 전부 "12px" 항목 통과, 남은 실패는 기존 "8px"·"16px"·상태바 텍스트 타이밍 플레이크뿐(이번 변경과 무관, 기존 항목들에 이미 기록됨). 반대 벤더 적대적 검증은 거치지 않음 — 값 하나를 바꾸고 그 검사 예외를 다섯 곳에 동일하게 반영한 기계적 변경으로 판단했다.

- **아이콘 테마에 `Simple`을 셋째로 추가 — sibling 프로젝트에서 이식 (사용자 요청, 2026-09-15)**
  - **요청**: 사용자가 `_clones\markdown_browser`를 확인해 달라고 했으나 실재하지 않아 확인 후 `D:\projects\markdown-viewer`로 정정, 그 프로젝트의 `Simple` 아이콘 테마(확장자 구분 없이 폴더·파일 각 하나의 윤곽선 그림만 쓰는 최소 테마)를 workbench-kit에 그대로 포함해 달라고 요청. 작업량을 먼저 확인해 보니 `SPEC.md` `FR-M9`가 "`Simple`은 없다"고 명시하고 있고(`D-11`), 이 문장을 그대로 검사하는 단언이 필수 반대 벤더 검증 대상인 Phase 4·Phase 7 스위트에 있었다. 이 발견을 보고하자 사용자가 "SPEC 수정해서 3개 테마로 추가 / 반대 벤더 검증 생략 → 구현 후 사용자가 직접 테스트"로 결정했고, 아이콘 테두리 색은 "VS Code Built-in의 폴더 아이콘과 같은 방식(색)"으로 하라고 지정했다.
  - **조치**: `docs/current/SPEC.md` `FR-M9`를 세 항목(`VS Code Built-in`·`VS Code Icons`·`Simple`)으로, `0.1절` 대체 목록(`v0.1 FR-Q1a`)도 "셋 중 하나"로 고쳤다. `DECISIONS.md`에 `D-18`(선택·근거·배제한 대안, D-11의 "아이콘 테마는 둘" 부분만 명시적으로 뒤집음)을 새로 썼다. `src/icons/simple.ts`(신규) — `FileIconResolver` 구현, `markdown-viewer`의 `shared/design/icons.svg`(`#icon-folder`·`#icon-folder-open`·`#icon-file`) 심볼 셋을 그대로 이식(같은 작성자 소스라 FR-Q4 라이선스 표기 불필요). 선 색은 `seti.ts`가 자기 폴더 아이콘에 쓰는 것과 정확히 같은 공식(다크 `#cccccc`·화이트 `#333333`·그레이 `#1f1f1f`)을 폴더·파일 모두에 적용. 열린 폴더 도안은 `<mask>`를 쓰는데, 트리에 열린 폴더가 여럿이면 고정 id가 충돌하므로 `resolve()` 호출마다 유일한 mask id를 만든다. `src/core/icontheme.ts`의 `FileIconThemeId`에 `'simple'` 추가. `src/core/tree.ts`(공통 코어) — `renderIconMarkup`의 `svg` 분기가 지금까지 `data-fg`를 안 넣고 있었다(vscode-icons는 고정 벤더 색이라 필요 없었음) — 감싸는 span에 `${fg}`를 추가해, 기존 `applyIconColors()`/`refreshThemeColors()`의 제네릭 `data-fg` 색칠 경로를 그대로 타게 했다(새 코드 추가 없이 기존 메커니즘 재사용). `src/main.ts` — 리졸버 등록, `View > Icon Theme` 목록에 `Simple` 추가.
  - **결과**: `View > Icon Theme`에서 `Simple`을 고르면 탐색기 트리의 폴더(닫힘/열림)·파일이 모두 단순 윤곽선으로 바뀌고, 세 색 테마를 오가면 그 선 색이 Seti 폴더 색과 같은 규칙으로 따라 바뀐다.
  - **뜻밖의 발견**: (1) `scripts/v02-phase4-suite.js`의 기존 단언이 "Icon Theme lists ... and **no Simple**"을 문자 그대로 검사하고 있어 정반대로 고쳐야 했다. (2) `scripts/phase7-suite.js`의 NFR-6 조합 검사(`distinctIconClasses`)가 아이콘 종류를 `.className`만으로 구분하고 있었는데, `vscode-icons`와 `simple`이 둘 다 `svg` 종류라 클래스 문자열이 같아 구분이 안 됐다 — `.innerHTML`을 더했더니 이번엔 반대로 실패했는데, 원인은 `svg` 아이콘의 색이(내가 방금 바꾼 대로) 감싸는 span 자신의 `data-fg`/`style`에 실리는 반면 codicon/font 아이콘은 자식 요소에 실려서 `.innerHTML`에 포함되는 차이 때문에, 두 종류가 "색까지 포함해서 같은지"를 서로 다른 기준으로 비교하고 있었다. 최종적으로 `outerHTML`을 쓰되 `data-fg`·`style="color:...`·Simple의 mask id를 전부 정규화해서 지운 뒤 비교하도록 고쳐, "아이콘 테마가 다른지"와 "색이 테마를 따라가는지"라는 서로 다른 두 검사를 확실히 분리했다(색 추종 자체는 같은 테스트의 `q2VisuallyValid`가 별도로 본다). (3) 1차로 남긴 영어 주석에 한글 인용문("Simple은 없다")을 그대로 남겼다가 `verify:phase3`의 "0 Korean characters" 검사에 걸려 다시 고쳤다.
  - **검증**: `npm run typecheck`·`build` 통과. `verify:phase2`·`verify:phase3`(한글 주석 검사 포함)·`verify:phase4`·`verify:v02-phase4`(`FR-M9` 재작성 단언, Electron·pywebview 양쪽에서 실제로 아이콘이 다시 그려지는 것까지 확인)·`verify:phase7`(NFR-6 9-조합 검사 포함)·`verify:v02-phase6`·`verify:dist` 새로 생긴 실패 0건 — 남은 실패는 전부 기존에 문서화된 것(8px·16px·FR-F11 대비 미달·Electron 상태바 타이밍 플레이크·v02-phase5의 "Electron runner exited non-zero" 프로세스 종료코드 플레이크)뿐. 반대 벤더 적대적 검증은 사용자 지시로 생략 — 사용자가 직접 테스트할 예정.

- **에디터 색을 VS Code 방식(`--vscode-*` 토큰)으로 껍데기가 정의 (사용자 요청, 2026-09-15)**
  - **요청**: monaco 연결을 "VS Code와 같은 방식 — 테마 및 글자색이 VS Code처럼" 적용하고, 최소한 plain text는 VS Code 같은 모습이 되게 할 것. 껍데기가 이 최소 설정을 미리 정해 두고 응용앱이 그대로 쓰게 할 것. 범위는 "권장"(plain text + 찾기 위젯), 회색 테마는 보간이 아니라 현재 회색 값 참조, 문법 강조는 이번엔 제외, 검증은 사용자가 직접.
  - **발견(착수 전 조사)**: monaco는 테마 색을 실행 시 `<style class="monaco-colors">`(140KB)로 주입하는데 이 앱의 CSP가 이를 차단한다 — `document.styleSheets`에 미등록임을 실측 확인(적용되는 시트는 앱 자신의 `<link>` 하나뿐). 그래서 `setTheme()`/`defineTheme()`으로는 색이 1픽셀도 안 바뀐다. 반면 vite가 번들한 monaco CSS는 정상 적용되고, 그 CSS는 색을 전부 `var(--vscode-…)`로만 참조한다(번들에서 119개 확인). 차단된 시트 안에는 `--vscode-*` 정의 1,770개 · `.mtk1~24` 토큰 색 24개 · 나머지(대부분 codicon) 881개가 들어 있었다.
  - **조치**: `DECISIONS.md`에 `D-19`를 새로 썼다. `style.css`에 `--vscode-*` 약 50개를 정의 — 공통분은 `:root` 한 블럭에서 앱 토큰(`--editor-bg`·`--text-color`·`--menu-hover-bg`·`--scrollbar-thumb` 등)에 연결하고, 앱 토큰에 대응이 없는 넷(줄번호 색, 찾기 강조 3종)만 테마 블럭마다 따로 뒀다. 회색 값은 이 테마의 기존 값(`#505050` = `--focus-ring`, `rgba(17,17,17,…)` = 스크롤바 베이스)을 그대로 썼다. `.mtk1`·커서·현재 줄 표시는 번들 CSS에 색 규칙 자체가 없어(차단된 쪽에만 있음) dockview 때와 같은 손복사 규칙으로 더했다 — 특히 커서는 이게 없으면 캐럿이 투명해 아예 안 보인다. `texteditor.ts`에 `setEditorColorTheme()`을 새로 두고(`vs`/`vs-dark` 클래스만 맞춤, 회색은 라이트 계열) `main.ts`의 `theme.onThemeChange`와 시작 시점에 연결했다.
  - **결과(실측)**: 세 테마 모두 의도한 값으로 렌더됨 — 배경 `#1f1f1f`/`#ffffff`/`#c0c0c0`, 본문 `#cccccc`/`#3b3b3b`/`#1a1a1a`, 비활성 줄번호 `#909090`/`#6c6c6c`/`#505050`, 활성 줄번호는 본문색, 캐럿은 본문색 2px, 현재 줄 테두리 `#2b2b2b`/`#e5e5e5`/`#9a9a9a`, 루트 클래스 `vs-dark`/`vs`/`vs`. 변경 전에는 배경이 투명이고 글자색은 앱 본문색을 상속하던 상태였다.
  - **뜻밖의 발견(이번 변경과 무관한 별개 결함) → 같은 세션에서 해결**: 같은 CSP가 monaco의 **인라인 style 속성**도 막고 있었다. 실측 결과 monaco가 CSSOM(`el.style.x = …`)으로 넣은 것(직렬화되며 콜론 뒤 공백이 붙음)은 적용되지만, HTML 문자열로 써 넣은 것(`"left:0px;width:38px;"`처럼 공백 없음)은 적용되지 않았다. 그 결과 줄번호 칸의 폭(38px→8px), 둘째 줄 래퍼의 높이(19px→0), 현재 줄 표시 폭(64px→2px)이 모두 무너져 **줄번호가 화면에 그려지지 않았다.** 값이 줄마다 동적이라 색 변수·해시·nonce·손복사 어느 것으로도 대체할 수 없어, 선택지는 CSP 완화 아니면 줄번호 포기뿐이었다. `.claude/rules/dockview-css.md`가 "CSP를 풀어서 인라인 스타일을 허용한다"를 하지 않는다로 적어 둔 항목이라 임의로 바꾸지 않고 세 선택지를 사용자에게 보고했고, 사용자가 완화를 선택했다.
  - **조치(CSP)**: `index.html`에 `style-src-attr 'unsafe-inline'` 한 항목만 더했다(`D-20`). `style-src 'self'`는 그대로라 `style-src-elem`이 계속 `'self'`로 내려가고, 주입된 `<style>`(monaco·dockview 테마 CSS)은 여전히 차단된다 — `D-19`의 `--vscode-*` 정의와 손복사 규칙이 계속 유일한 색 출처다. 규칙 문서 `.claude/rules/dockview-css.md`도 이 구분(속성은 열림, 요소는 계속 차단)을 적도록 함께 고쳤다. 자동 승인 분류기가 이 변경을 "보안 약화"로 한 번 차단해, 사용자 재확인을 받고 진행했다.
  - **결과(CSP 완화 후 재측정)**: 줄번호가 `1`(활성 `#cccccc`)·`2`(비활성 `#909090`) 각각 **폭 38px·19px 간격**으로 정상 렌더된다(완화 전 8px·높이 0). 본문 줄도 y=70/89로 정상 배치. 스크린샷으로 줄번호 `1`이 우측 정렬로 칸 안에 들어가고 현재 줄 테두리가 그 줄을 감싸는 것까지 확인했다. 같은 실행에서 `style.monaco-colors`가 여전히 `document.styleSheets`에 미등록임(차단 유지)을 함께 확인했다.
  - **검증**: `npm run typecheck`·`build` 통과. 세 테마의 계산된 색·캐럿·줄번호 색, CSP 완화 후 줄번호 기하를 Electron에서 실측(위 결과). 검사 스크립트 중 CSP 문자열을 정확히 단언하는 것은 없어(존재 여부만 봄) 갱신할 테스트는 없었다. 기능 검증은 사용자 지시대로 사용자가 직접 수행. 반대 벤더 적대적 검증은 사용자 지시로 생략.

## 3. 마감 요약 (v0.2, 2026-09-16)

| 항목 | 내용 |
| --- | --- |
| 계획 대비 | task 34 / 완료 34 (`backlog.json` 전건 `done`). PLAN.md Phase 1~7 전부 완료, 필수 게이트(Phase 1·4·7) 반대 벤더 검증 `docs/reviews/`에 기록 |
| 계획 외 개선 | 26건 (§2). 검토 루프에서 나온 UX 보정 다수 — 탐색기·탭·테마·아이콘 사용성, pywebview 영속화 결함, 상태표시줄 경로 연동, 최대화 아이콘 토글, `Simple` 아이콘 테마 이식, monaco `--vscode-*` 색 연결(CSP 완화 포함) |
| 남긴 것 | ① 금지 값(v0.1 D-29) `8px`·`16px` 2건 — 트리 세로선, 사용자 결정으로 현재 값 유지(2026-09-14). ② `verify:v02-phase2`의 FR-F11 sel/sidebar 대비 미달 5건 — 발견 시점부터 미달로 기록, 해소 안 함. ③ `verify:v02-phase5`의 "Electron runner exited non-zero", `verify:dist`의 상태바 텍스트 타이밍 — 원본 대조로 재현되는 기존 환경 플레이크, 이번 버전 변경과 무관. ④ pywebview 최대화 아이콘 토글·localStorage 영속화 재확인 — 스모크 테스트만 했고 버튼을 직접 눌러보는 재확인은 사용자 몫으로 남김. ⑤ monaco 문법 강조(`.mtk*` 토큰) — D-19에서 범위 밖으로 명시적으로 제외 |
| 다음 버전으로 | 위 ①·②·④·⑤가 후보. 검토 루프 중 새로 나온 확장 아이디어는 없었음 — `docs/refs/next-version.md`(v0.1에서 넘어온 목록)와 대조해 다음 버전 `BRIEF.md` 작성 시 참고 |
