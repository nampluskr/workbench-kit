# RESERVED-KEYS — 껍데기가 먼저 가져가는 키 (FR-R1 · FR-R1a · D-19)

> 이 문서가 유일한 예약 키 목록이다(FR-R1). 여기 없는 키는 전부 앱(탭 안의 보기) 몫이다.
> 포커스가 탭 안에 있을 때 이 목록에 없는 키를 누르면 껍데기 쪽 처리 기록이 0건이고
> 그 키는 탭의 보기가 받는다(FR-I6).

## 1. 전역 — 포커스와 무관하게 껍데기가 가져간다

| 키 | 동작 | 근거 |
| --- | --- | --- |
| `Ctrl+O` | 폴더 열기 (`File > Open Folder...`) | FR-A1, D-16, v0.2 FR-M4 |
| `Ctrl+W` | 활성 탭 닫기 (`File > Close Active Tab`) | FR-N6b, v0.2 FR-M4 |
| `Ctrl+N` | 활성 칸에 새 빈 탭 열기 (`New Tab`) | 사용자 요청 (2026-09-18) |
| `Ctrl+\` | 좌우 스플릿 (`File > Split Right`) | FR-D1 |
| `Ctrl+K Ctrl+\` | 상하 스플릿 (`File > Split Down`) | 사용자 요청 (2026-09-18) |
| `Ctrl+K W` | 활성 그룹 모든 탭 닫기 (`File > Close Editor Group`) | 사용자 요청 (2026-09-18) |
| `Ctrl+PageDown` / `Ctrl+PageUp` | 활성 칸 안에서 다음 / 이전 탭으로 전환 | 사용자 요청 (2026-09-18) |
| `Ctrl+Tab` / `Ctrl+Shift+Tab` | 활성 칸 안에서 다음 / 이전 탭으로 전환 (호환) | v0.2 FR-F2, D-10 |
| `Ctrl+0` | 탐색기로 포커스 이동 (사이드바 숨겨져 있으면 열고 포커스) | 사용자 요청 (2026-09-18) |
| `Ctrl+1` | 첫 번째 에디터 그룹으로 포커스 이동 | 사용자 요청 (2026-09-18) |
| `Ctrl+2` | 두 번째 에디터 그룹으로 포커스 이동 (1개뿐이면 우측 분할 생성) | 사용자 요청 (2026-09-18) |
| `Ctrl+B` | 현재 표시된 폴더탭·탐색기를 함께 감추고, 다음 입력에 직전 조합으로 복원 | v0.3 사용자 요청 (2026-09-18) |
| `Alt+F4` | 끝내기 (`File > Exit`) — 메뉴와 같은 닫기 확인 경로. `Alt` 없는 `F4`는 예약하지 않는다 | v0.2 FR-M4 · FR-M5 |
| `F10` | 햄버거 메뉴 열기/닫기 | FR-N5, D-7 |
| `F11` | Zen 모드 전환 (`View > Zen Mode`) | D-12, v0.2 FR-M4 |
| `Escape` | 열린 메뉴 닫기, Zen 모드 나가기, 또는 대기 중인 키 시퀀스(`Ctrl+K`) 취소 | D-12 |

`Tab` / `Shift+Tab`은 **예약하지 않는다.** v0.1 그대로 브라우저 기본 포커스 이동(세로 띠 ·
탭 줄 버튼 사이)과 탭 안 보기의 입력(에디터 들여쓰기 등)에 쓰인다. 영역 순환을 `Tab`에
두지 않은 이유는 이 두 경로를 깨지 않기 위해서다(v0.2 D-10, 사람 결정 2026-09-10).

## 1b. 포커스가 탭 안 보기 **밖**에 있을 때만 (v0.2 D-10)

| 키 | 동작 | 근거 |
| --- | --- | --- |
| `F6` / `Shift+F6` | 다음 / 이전 영역(보이는 트리 · 각 칸)으로 포커스 이동. 숨겨진 영역은 건너뛴다 | v0.2 FR-F1 · FR-F4, D-10 |

포커스가 탭 안 보기 **안**(에디터 · 앱 화면)에 있으면 `F6`은 v0.1 `FR-I6`대로 그 보기가
받는다 — 껍데기는 막지도 처리하지도 않는다. 트리 · 탭 줄 · 칸 자체(`F6`이 옮겨 놓은
자리)에서만 영역을 순환한다.

## 2. 메뉴가 열려 있을 때만

| 키 | 동작 |
| --- | --- |
| `ArrowUp` / `ArrowDown` | 항목 사이 이동 (하위 메뉴가 열려 있으면 그 안에서) |
| `ArrowRight` | 하위 메뉴가 있는 항목(`Recent Folders` · `Color Theme` · `Icon Theme`)이면 그 하위 메뉴를 연다. 아니면 다음 묶음(File → View → Help) |
| `ArrowLeft` | 하위 메뉴가 열려 있으면 닫는다. 아니면 이전 묶음 |
| `Enter` | 포커스된 항목 실행 (하위 메뉴가 있는 항목이면 연다) |
| `Delete` | `Recent Folders` 하위 메뉴에서 포커스된 경로를 목록에서 지운다 (v0.2 FR-M7) |
| `Escape` | 메뉴 닫기 |

## 2b. 우클릭 메뉴가 열려 있을 때만 (WK-030, D-22)

| 키 | 동작 |
| --- | --- |
| `ArrowUp` / `ArrowDown` | 항목 사이 이동 |
| `Enter` | 포커스된 항목 실행 후 닫기 |
| `Escape` | 메뉴 닫기 |

우클릭 메뉴와 햄버거 메뉴는 상호 배타적이다 — 하나가 열리면 다른 하나는 자동으로
닫힌다(`workbench:menu-open` · `workbench:contextmenu-open` 이벤트로 조율).

## 3. 트리에 포커스가 있을 때만 (FR-A2 ~ FR-A19, D-9)

| 키 | 동작 |
| --- | --- |
| `ArrowUp` / `ArrowDown` | 포커스 1칸 이동 |
| `ArrowRight` | 접힌 폴더 펼침 / 펼쳐진 폴더 첫 자식으로 이동 |
| `ArrowLeft` | 펼친 폴더 접기 / 접힌 폴더·리프는 부모로 이동 |
| `Ctrl+ArrowLeft` | 모두 접기 |
| `Space` | 토글(펼침/접힘) |
| `Enter` | 고른 항목 확정 — 이미 임시로 열려 있으면 그 탭을 확정하고, 아니면 바로 확정된 탭으로 연다 (v0.2 FR-P7 · v0.1 FR-A6 대체) |
| `Ctrl+Enter` | 옆 칸에 열기 (FR-A14) |
| `Ctrl+Shift+Enter` | 선택 토글 |
| `Home` / `End` | 처음/끝으로 이동 |
| `PageUp` / `PageDown` | 화면 단위 이동 |
| `Shift+ArrowUp` / `Shift+ArrowDown` | 연속 범위 선택 |
| `Ctrl+ArrowUp` / `Ctrl+ArrowDown` | 포커스 불변 스크롤 |
| `Ctrl+A` | 전체 선택 |
| `Escape` | 선택 해제 (또는 찾기 위젯이 열려 있으면 찾기 위젯 닫기) |
| `F3` / `Ctrl+Alt+F` | 트리 안에서 찾기 열기 |

## 3b. 탐색기 폭 조절 손잡이에 포커스가 있을 때만 (v0.2 FR-X5 · NFR-8)

경계 손잡이(`#sidebar-resize-handle`)는 `tabindex="0"`이라 `Tab`으로 포커스한다.

| 키 | 동작 |
| --- | --- |
| `ArrowLeft` / `ArrowRight` | 탐색기 폭을 한 단계(16px) 줄임 / 늘림 |
| `Home` | 최소 폭으로 |
| `End` | 창 폭의 40%로 |

Zen 모드이거나 탐색기가 숨겨져 있으면 손잡이가 안 보이고 이 키들도 동작하지 않는다.
조절한 폭은 저장하지 않는다(D-8) — 다시 열면 초기값이다.

## 4. 껍데기가 **가져가지 않는다** — 흔히 오해하는 여섯 (FR-R1a)

탭에 포커스가 있을 때 아래 여섯은 껍데기가 손대지 않고 그대로 탭의 보기가 받는다.
저장·이름 바꾸기·삭제·되돌리기·복사·붙이기는 전부 리소스 종류를 아는 앱의 일이다
(D-18 · D-22 · D-30).

| 키 | 앱 몫 |
| --- | --- |
| `Ctrl+S` | 저장 |
| `F2` | 이름 바꾸기 |
| `Delete` | 삭제 |
| `Ctrl+Z` / `Ctrl+Y` | 되돌리기 / 다시하기 |
| `Ctrl+C` / `Ctrl+V` | 복사 / 붙이기 |

## 5. 마우스 없이 부르는 경로 (v0.2 NFR-8, v0.1 NFR-7)

1절에 나오는 모든 명령은 각각 마우스 없이 실행하는 경로가 최소 1개 있다.

| 자리 | 경로 |
| --- | --- |
| 메뉴(File · View · Help) | `F10`으로 열고, 화살표 키로 항목을 옮기고, `Enter`로 실행한다 |
| 세로 띠 | 네이티브 `<button>`이라 `Tab`으로 포커스하고 `Enter`/`Space`로 누른다 |
| 탭 줄(Split Right · Split Down · New Tab) | 네이티브 `<button>`이라 `Tab`으로 포커스하고 `Enter`/`Space`로 누른다 |
| 탐색기 뷰 액션(New File · New Folder · Refresh · Collapse All) | 네이티브 `<button>`이라 `Tab`으로 포커스하고 `Enter`/`Space`로 누른다 (v0.2 FR-X2 · FR-X4) |
| 탐색기 폭 조절 | 경계 손잡이를 `Tab`으로 포커스하고 3b절의 키로 조절한다 (v0.2 FR-X5) |

이 경로들에 쓰이는 키는 전부 위 1 ~ 3b절에 있다. 이 목록 밖의 키가 명령 실행에 쓰이는
경우는 없다.
