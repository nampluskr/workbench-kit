...
console.log('=== workbench-kit: v0.2 Phase 2 (선택과 포커스 표시) Verification ===');
let failures = 0;
 ...
 */
const REQUIRED_V02_PHASE2_ASSERTIONS = Object.freeze([
  'INIT',
  'V2P2-FIXTURE',
  'V2P2-FR-P9',
  'V2P2-FR-F1',
...
const electron = runHost(
  'Electron',
  'npx.cmd electron scripts/v02-phase2-electron-runner.cjs',
  '[electron] v0.2 Phase 2 Results:'
);
const pywebview = runHost(
  'pywebview',
  '"C:\\winpython\\WPy64-31180_cpu\\python-3.11.8.amd64\\python.exe" -m src.hosts.pywebview.main --v02-phase2-test',
  '[pywebview] v0.2 Phase 2 Results:'
);

for (const [label, result] of [['Electron', electron], ['pywebview', pywebview]]) {
  if (!result) continue;
  assert(result.success === true, `${label}: all v0.2 Phase 2 assertions passed`);
  const ids = result.results.map((r) => r.id);
  assert(
    ids.length === REQUIRED_V02_PHASE2_ASSERTIONS.length,
    `${label}: executed exactly ${REQUIRED_V02_PHASE2_ASSERTIONS.length} assertions (got ${ids.length})`
  );
  for (const required of REQUIRED_V02_PHASE2_ASSERTIONS) {
...
if (failures > 0) {
  console.error(`\nFAILED: ${failures} v0.2 Phase 2 check(s) failed.`);
  process.exit(1);
}
console.log('\nSUCCESS: All v0.2 Phase 2 quality checks passed (0 failures).');
