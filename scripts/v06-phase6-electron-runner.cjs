...
app.setPath('userData', path.join(os.tmpdir(), 'wb-v06p6-userdata-' + Math.random().toString(36).slice(2)));

const rootDir = path.resolve(__dirname, '..');
const distIndexPath = path.join(rootDir, 'dist', 'index.html');
const suitePath = path.join(__dirname, 'v06-phase6-suite.js');
...
app.whenReady().then(async () => {
  const fixtureDir = makeFixture('wb-v06p6-fixture-');
  const fixtureDir2 = makeFixture('wb-v06p6-fixture2-');
  ...
  try {
    ...
    const testResult = await win.webContents.executeJavaScript(`
      ...
      ${suiteCode}
      window.__runV06Phase6Suite();
    `);
    console.log('[electron] v0.3 Phase 6 Results: ' + JSON.stringify(testResult));
    assert(testResult && testResult.success === true, 'All v0.3 Phase 6 Electron assertions passed');
  } catch (err) {
