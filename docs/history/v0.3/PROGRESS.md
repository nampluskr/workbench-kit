...
**계획 대비** — `backlog.json`의 28개 task(`WK-083` ~ `WK-110`, Phase 1~6) 전부
`done`. 필수 통과 Phase(1·4·6)는 각각 반대 벤더 적대적 검증 기록이 있다
(`docs/reviews/A16.md`·`A19.md`·`A21.md`) — Phase 4(A19)는 3회차에도 Critical
2건이 남아 3회 제한 소진 후 직접 수정 + 회귀 테스트로 해소했으나 4회차 벤더
재검증은 받지 못한 채 사용자 지시("모든 phase를 순차적으로 진행")로 넘어간
전례가 있다(같은 패턴이 이번 마감 직전의 `A29`에서도 반복돼 이번엔 사용자가
명시적으로 승인했다).

**계획 외 개선** — 세션 전체에서 다수(파일 필터·Edit 메뉴·Save 계열부터 시작해
약 20건 이상) 발생했고, 이번 마감 직전 4건이 특히 크다: F3/F4 보기·편집 단축키,
Help > About 스크롤 제거, Explorer Find 버튼 + D2Coding 글꼴 통일 + 다크 테마
메뉴 구분선 대비(`A28`), 탐색기 삭제 기능(Delete 키 + 우클릭, 전역 게이트,
`A29`). 전부 `npm run typecheck`·`npm run build`·관련 `verify:*` 통과, 회귀 없음.

**남긴 것** — `A19`(Phase 4)의 4회차 미실행, `A29`(삭제 기능)의 마지막 Critical
수정에 대한 벤더 재검증 미실행 — 둘 다 회귀 테스트로 실측 확인했지만 독립적인
반대 벤더 재확인은 없다. 탐색기 삭제가 **비활성** Folder Tab이 가리키는 경로를
지웠을 때는 그 탭을 즉시 갱신하지 않고, 다음에 그 탭을 열 때 기존 D-6 오류 탭
복구 경로로만 드러난다(의도적으로 범위 밖에 둠, `A29` 참조).

**다음 버전으로** — 사용자 요청으로 검토한 "Activity Bar 마크다운 아이콘 +
렌더링 미리보기" 기능은 이번 버전 범위 밖으로 판단해 v0.4로 넘긴다. 껍데기
자체는 이미 이 기능에 필요한 것을 다 갖추고 있다(`docs/refs/next-version.md`의
"마크다운 에디터" 항목, "남은 것: 없다") — 필요한 건 전부 앱 레이어(`src/presets/`)
새 코드다. 상세는 `docs/refs/next-version.md`에 별도로 남긴다.

**DECISIONS 승격** — `D-1`~`D-15` 중 승격 후보로 `D-9`(작업 폴더 루트는 껍데기
인프라, 리소스 종류와 다른 층)와 `D-15`(텍스트/바이너리 판별은 앱 레이어)를
제안했으나, **사용자가 "아무것도 승격하지 않음"으로 결정**했다(2026-09-25).
`CLAUDE.md`·`.claude/rules/`는 변경 없음 — `D-1`~`D-15` 전부 `docs/history/v0.3/
DECISIONS.md`에 근거로만 남는다.
