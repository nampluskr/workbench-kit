...
workbench-kit은 여러 데스크톱 앱이 공통으로 쓰는 **베이스 앱**이다. 창, 메뉴, 탐색기
트리, 좌우로 가른 패널, 탭 줄, 상태 표시줄, 설정 저장, 실행 환경 연결을 갖추고, 그
자체로 쓸 수 있는 stand-alone 앱으로 필수·공통 기능(멀티터미널, 마크다운 렌더링 등)을
구현해 둔다. 새 앱은 이 베이스 앱을 복사해 파생하고, 그 앱에서만 달라지는 소수의
자리만 갈아끼워 완성한다.

앱은 가볍게 둔다 — 그 도메인에 꼭 필요한 기능만 담는다. 화면은 VS Code가 다듬어 온
구성과 동작을 기준으로 삼고, 파이썬이 필요한 앱은 pywebview, 필요 없는 앱은 Electron
갈래로 같은 화면을 띄운다.
