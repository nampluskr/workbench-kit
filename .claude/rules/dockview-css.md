...
`style-src 'self'`에는 `'unsafe-inline'`이 없다. 그래서 **주입된 `<style>`뿐 아니라
요소의 `style="..."` 속성도 브라우저가 무시한다.** 렌더 코드에서 기하·색을 인라인
`style`로 넣으면 DOM에는 남지만 화면에는 반영되지 않는다 — v0.2 Phase 5에서 트리
들여쓰기(`style="padding-left"`)와 세로 정렬선(`style="left"`)이 통째로 안 그려지고
있던 것을 뒤늦게 찾았다(v0.1 검사는 MockElement나 HTML 문자열만 봐서 통과).

**하지 않는다**: 렌더 HTML 문자열에 `style="..."`를 넣어 위치·크기·색을 정한다.
대신 클래스, 구조적 요소(고정폭 스페이서), `::before`/`::after`로 표현한다.
