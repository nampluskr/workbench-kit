...
app.setPath('userData', path.join(os.tmpdir(), 'wb-v02p1-userdata-' + Math.random().toString(36).slice(2)));

const rootDir = path.resolve(__dirname, '..');
const distIndexPath = path.join(rootDir, 'dist', 'index.html');
const suitePath = path.join(__dirname, 'v02-phase1-suite.js');
...
function makeFixture() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'wb-v02p1-fixture-'));
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    const testResult = await win.webContents.executeJavaScript(`
      ...
      ${suiteCode}
      window.__runV02Phase1Suite();
    `);
    console.log('[electron] v0.2 Phase 1 Results: ' + JSON.stringify(testResult));
    assert(testResult && testResult.success === true, 'All v0.2 Phase 1 Electron assertions passed');
  } catch (err) {
